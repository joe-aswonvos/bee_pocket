...
def item_instances(request, beepocket_id):
    """Retrieve and return item instances for a specific BeePocket."""
    user = request.user
...
def approve_item_instance(request, instance_id):
    ...
    except ItemInstance.DoesNotExist as e:
        messages.error(request, f'Error: {str(e)}')
    except Exception as e:
        messages.error(request, f'An unexpected error occurred: {str(e)}')
    return redirect('create_item')
...
def edit_item_instance(request, instance_id):
    """Edit an existing item instance."""
    instance = get_object_or_404(ItemInstance, id=instance_id)
...
def new_item_details(request, item_id):
    """Retrieve and return details of a specific item."""
    try:
        item = get_object_or_404(Item, id=item_id)
        data = {
        ...
        return JsonResponse({'error': f'Error: {str(e)}'}, status=404)
    except Exception as e:
